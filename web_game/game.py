...
if the_depths_bg._surf.get_size() != (1100, 600):
...
if the_intro_bg._surf.get_size() != (1100, 600):
...
# Try to start music on load (Chrome often allows it). If the browser
# blocks autoplay, the first click will unlock and start it.
_audio_loaded = False
_audio_unlocked = False
click_sound = None
powder_collect_sound = None
tool_break_sound = None


def _load_audio_assets():
    global _audio_loaded, click_sound, powder_collect_sound, tool_break_sound
    if _audio_loaded:
        return
    try:
        from pgzcompat import sound_path
        click_sound = pygame.mixer.Sound(sound_path("click"))
        click_sound.set_volume(0.5)
        powder_collect_sound = pygame.mixer.Sound(sound_path("powder_collect"))
        powder_collect_sound.set_volume(0.6)
        tool_break_sound = pygame.mixer.Sound(sound_path("tool_break"))
        tool_break_sound.set_volume(0.6)
        print("sfx loaded ok", flush=True)
    except Exception as e:
        print("sfx init failed:", e)
        import traceback
        traceback.print_exc()
    _audio_loaded = True


def _start_scene_music():
    try:
        music.set_volume(0.4)
        if show_intro_popup:
            music.play("intro")
        elif show_end_game_popup:
            music.play("end_game")
        else:
...
def _ensure_audio(start_music=True):
    """Load SFX, unlock browser audio, and start scene music if needed."""
    global _audio_unlocked
    _load_audio_assets()
    try:
        from pgzcompat import unlock_browser_audio
        unlock_browser_audio()
    except Exception as e:
        print("unlock_browser_audio failed:", e)
...
print("mineral_explorer: init complete", flush=True)
...
def _play_click():
    try:
        ...
        print("click sound failed:", e)

def _play_powder():
    try:
        if powder_collect_sound is not None:
            powder_collect_sound.play()
    except Exception as e:
        print("powder sound failed:", e)

def _play_break():
    try:
        if tool_break_sound is not None:
            tool_break_sound.play()
    except Exception as e:
        print("break sound failed:", e)


def restart_game():
    """Reset all gameplay state and return to the intro screen."""
    global coins, selected_tool, clicked_mineral, result_image, result_message
    global show_intro_popup, show_mineral_popup, show_store_popup
    global show_result_popup, show_end_game_popup, quit_requested
    global collected_talc, collected_fluorite, collected_apatite, collected_silver
    global collected_opal, collected_topaz, collected_amethyst, collected_tungsten_carbide
    global show_talc, show_fluorite, show_apatite, show_silver
    global show_opal, show_topaz, show_amethyst, show_tungsten_carbide
    ...
    try:
        ...
        print("restart music failed:", e)
    print("game restarted to intro", flush=True)
...
def on_mouse_down(pos):
    ...
    # Ending — restart for another run (web can't usefully "quit")
    if show_end_game_popup:
...
def draw_story(story, x, y, max_width, fontsize=24, highlight_color="orange"):
    ...
    # Use the same font backend as screen.draw.text (SysFont) so wrap metrics match glyphs.
